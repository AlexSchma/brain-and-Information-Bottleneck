...
from data.create_data import load_data, separate_data, convert_brib_to_s2vgraph
from graphCNNb import GraphCNN
from pre_subgraph import MLP_subgraph
from sklearn.metrics import confusion_matrix
from data.dataset import BrIB_RESTfMRIDataset, BrIB_preprocessedBrainIB_RESTfMRIDataset
import random
...
#     split="full"
# )
full_dataset = BrIB_preprocessedBrainIB_RESTfMRIDataset(
    metadata_path="../../Data/metadata.csv",
    data_dir="../../Data/fMRI/AAL",
    split="full"
)
...
import random
graphs = convert_brib_to_s2vgraph(full_dataset)
random.shuffle(graphs)



num_folds = 10
args.epochs = 100  # override epochs to 100
num_classes = 2
all_fold_results = []
...
for fold_idx in range(num_folds):
    ...
    print(f"Fold {fold_idx + 1} - Train dataset size: {len(train_graphs)}, Test dataset size: {len(test_graphs)}")
    model = GraphCNN(
...
output_file = "MDD_results_V1prepros.txt"
output_path = os.path.join(output_dir, output_file)
with open(output_path, 'w') as f:
    f.write("=== Cross-validation Results ===\n")
    for i, res in enumerate(all_fold_results):
        f.write(f"Fold {i+1}: Acc: {res['acc_test']:.4f}, F1: {res['test_f1s']:.4f}, MCC: {res['test_mcc']:.4f}\n")
    f.write(f"\nAverage Test Accuracy: {avg_acc:.4f}\n")
    f.write(f"Average Test F1-Score: {avg_f1:.4f}\n")
    f.write(f"Average Test MCC: {avg_mcc:.4f}\n")
