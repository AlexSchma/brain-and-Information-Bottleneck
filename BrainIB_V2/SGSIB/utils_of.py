...
def train(args, model, train_dataset, optimizer, epoch, SG_model, device, criterion=nn.CrossEntropyLoss()):
    ...
    single_batch_graphs = train_dataset[:args.batch_size]
    batch_graph = next(iter(DataLoader(single_batch_graphs, batch_size=len(single_batch_graphs))))
    subgraphs = copy.deepcopy(single_batch_graphs)
    ...
    for _ in pbar:
        ...
        mi_loss = calculate_MI(embeddings, positive, sigma1 ** 2, sigma2 ** 2) / len(subgraphs)
        labels = batch_graph.y.view(-1).to(device)

        regularization_loss = sum(torch.sum(torch.abs(param)) for param in model.parameters())
    ...
    average_miloss = miloss_accum / total_iters
...
def test(args, model, train_dataset, test_dataset, SG_model, device, criterion=nn.CrossEntropyLoss()):
    ...
    # Use only the batch used for overfitting during training
    train_dataset = copy.deepcopy(train_dataset[:args.batch_size])
